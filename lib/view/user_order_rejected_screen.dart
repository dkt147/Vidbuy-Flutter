...
class _UserRejectedScreenState extends State<UserRejectedScreen> {
  ...
  void initState() {
    ...
    userOrdersViewModel.fetchUserRejectedVideoList();
  }
    ...
    return Scaffold(
      body: ChangeNotifierProvider(
        ...
        child:
          ...
          return Column(
            children: [
              if (viewModel.isRejectedOrderFetching &&
                  viewModel.rejectedOrders.isEmpty)
                const Expanded(
                  child: Center(
                    child: CircularProgressIndicator(),
                  ),
                )
              else if (!viewModel.isRejectedOrderFetching &&
                  viewModel.rejectedOrders.isEmpty)
                Expanded(
                  child: Center(
                    child: Column(
                  ...
                  child: NotificationListener<ScrollNotification>(
                    onNotification: (ScrollNotification scrollInfo) {
                      if (scrollInfo.metrics.pixels ==
                          ...
                          !viewModel.isRejectedOrderFetching &&
                          viewModel.hasRejectedOrdersMoreData) {
                        viewModel.fetchUserRejectedVideoList();
                        return true;
                      }
                      return false;
                    },
                    child: Padding(
                      ...
                      child: ListView.builder(
                        itemCount: viewModel.rejectedOrders.length +
                            (viewModel.isRejectedOrderFetching ? 1 : 0),
                        itemBuilder: (context, index) {
                          if (index == viewModel.rejectedOrders.length) {
                            return const Center(
                              child: Padding(
                          ...
                          final influencer = viewModel.rejectedOrders[index];
                          return Padding(
